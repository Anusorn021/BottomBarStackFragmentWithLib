 ...
 * Copyright (c) 2016 Iiro Krankka (https://github.com/roughike/BottomBar).
 *
...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    public void setOnTabSelectListener(@NonNull OnTabSelectListener listener, boolean shouldFireInitially) {
        ...
        if (shouldFireInitially && getTabCount() > 0) {
            listener.onTabSelected(getCurrentTabId(),getCurrentTabId());
        }
    }
    ...
    private void updateSelectedTab(int newPosition) {
        ...
        int preTabId = getTabAtPosition(currentTabPosition).getId();

        if (newPosition != currentTabPosition) {
            if (onTabSelectListener != null) {
                onTabSelectListener.onTabSelected(preTabId,newTabId);
            }
